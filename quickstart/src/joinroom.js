...
function toggleScreenShareButtons() {
  if ($screen_share.is(':hidden')){
    $screen_share.show();  
  ...
  if ($stopscreencapture.is(':hidden')){
...
async function joinRoom(token, connectOptions) {
  ...
  $screen_share.click(async function() {
    try {
        ...
        screenTrack =await helpers.createScreenTrack(720, 1280);
      
        screenTrack.attach($activeVideo.get(0));
        await room.localParticipant.publishTrack(screenTrack);
        screenTrack.on('stopped', () => {
          if (room) {
            room.localParticipant.unpublishTrack(screenTrack);
          }
          toggleScreenShareButtons();
          });
